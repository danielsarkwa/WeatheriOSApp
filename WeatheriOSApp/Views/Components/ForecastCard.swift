...
//  WeatheriOSApp
//
//  Created by Daniel Yeboah on 24.6.2023.
//
...
struct ForecastCard: View {
    ...
    var isActive: Bool { // computered property
        if forecastPeriod == ForecastPeriod.hourly {
            let isThisHour = Calendar.current.isDate(.now, equalTo: forecast.date, toGranularity: .hour) // check if the forecast hour is equal to the the current hour // return true or false
            return isThisHour
        } else {
            let isToday = Calendar.current.isDate(.now, equalTo: forecast.date, toGranularity: .day) // check if the forecast day is equal to the the current day // return true or false
            return isToday
        }
    }
